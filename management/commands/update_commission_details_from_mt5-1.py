        ...
        for account in accounts:
            ...
            for deal in closed_trades:
                ...
                if not trade_id or trade_id in ('None', ''):
                    continue
                # Compute lot_size from Volume (MT5 stores volume as units where 10000 = 1.0 lot)
                volume = getattr(deal, 'Volume', 0) or 0
                try:
                    lot_size = float(volume) / 10000.0 if float(volume) > 0 else 0.0
                except Exception:
                    lot_size = 0.0
                # Profit
                try:
