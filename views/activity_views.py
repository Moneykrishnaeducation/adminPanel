...
def login_view(request):
    ...
    if current_ip:
        ...
        if _check_rate_limit(rl_key, login_limit, 60):
            return JsonResponse({'error': 'Too many login attempts from your IP. Try again later.'}, status=429)
    ...
    if last_ip and current_ip and last_ip != current_ip:
        try:
            # Before generating/sending OTP, enforce per-IP and per-email send limits
            ...
            email_rl_key = f"rl:otp:send:email:{user.email.strip().lower()}"
            ip_rl_key = f"rl:otp:send:ip:{current_ip}"
            if current_ip and _check_rate_limit(ip_rl_key, otp_ip_limit, 3600):
                return JsonResponse({'error': 'Too many OTP send attempts from your IP. Try again later.'}, status=429)
            if _check_rate_limit(email_rl_key, otp_send_limit, 3600):
                return JsonResponse({'error': 'Too many OTP send attempts for this account. Try again later.'}, status=429)
    ...
    base_url = 'http://admin.localhost:8000'  # Adjust this to your actual base URL
    if user.is_superuser:
        redirect_url = f'{base_url}/static/admin/admin/main.html'
    elif getattr(user, 'manager_admin_status', '') == 'Manager':
        redirect_url = f'{base_url}/static/admin/manager/main.html'
    else:
        redirect_url = f'{base_url}/static/admin/admin/main.html'
    ...
    # Set secure HttpOnly SameSite cookies for access/refresh tokens so clients can use
    # credentialed requests. Use DEBUG to decide Secure flag.
    try:
        secure_flag = not settings.DEBUG
        cookie_domain = getattr(settings, 'COOKIE_DOMAIN', '.localhost')  # Use COOKIE_DOMAIN from settings
        try:
        ...
        # Set cookies: jwt_token and access_token for compatibility, refresh_token for rotation
        # HttpOnly=True for security - JavaScript cannot read, but browser sends automatically with credentials: include
        # domain parameter allows cookies to be sent to all subdomains (e.g. admin.localhost)
        response.set_cookie('jwt_token', access_token, httponly=True, secure=secure_flag, samesite='Strict', path='/', domain=cookie_domain, max_age=access_max_age)
        response.set_cookie('access_token', access_token, httponly=True, secure=secure_flag, samesite='Strict', path='/', domain=cookie_domain, max_age=access_max_age)
        response.set_cookie('accessToken', access_token, httponly=True, secure=secure_flag, samesite='Strict', path='/', domain=cookie_domain, max_age=access_max_age)
        response.set_cookie('refresh_token', str(refresh), httponly=True, secure=secure_flag, samesite='Strict', path='/', domain=cookie_domain, max_age=refresh_max_age)
        response.set_cookie('refreshToken', str(refresh), httponly=True, secure=secure_flag, samesite='Strict', path='/', domain=cookie_domain, max_age=refresh_max_age)
        try:
            # UI-facing cookies (not HttpOnly) for display — 7 day expiry
            ui_cookie_params = {
                'httponly': False,
                'secure': secure_flag,
                'samesite': 'Strict',
            ...
            response.set_cookie('userName', user_name or '', expires=(timezone.now() + timedelta(days=7)), **ui_cookie_params)
            response.set_cookie('userEmail', user.email or '', expires=(timezone.now() + timedelta(days=7)), **ui_cookie_params)
            response.set_cookie('userRole', getattr(user, 'manager_admin_status', '') or '', expires=(timezone.now() + timedelta(days=7)), **ui_cookie_params)
        except Exception:
            logger.exception('Failed to set UI cookies in login_view')
    except Exception:
        logger.exception('Failed to set auth cookies in login_view')
...
def refresh_and_set_cookie_view(request):
    ...
    try:
        ...
        try:
            ...
            cookie_domain = getattr(settings, 'COOKIE_DOMAIN', '.localhost')  # Use COOKIE_DOMAIN from settings
            resp.set_cookie('jwt_token', new_access, httponly=True, secure=secure_flag, samesite='Strict', path='/', domain=cookie_domain, max_age=access_max_age)
            resp.set_cookie('access_token', new_access, httponly=True, secure=secure_flag, samesite='Strict', path='/', domain=cookie_domain, max_age=access_max_age)
            resp.set_cookie('accessToken', new_access, httponly=True, secure=secure_flag, samesite='Strict', path='/', domain=cookie_domain, max_age=access_max_age)
            resp.set_cookie('refresh_token', new_refresh_str, httponly=True, secure=secure_flag, samesite='Strict', path='/', domain=cookie_domain, max_age=refresh_max_age)
            resp.set_cookie('refreshToken', new_refresh_str, httponly=True, secure=secure_flag, samesite='Strict', path='/', domain=cookie_domain, max_age=refresh_max_age)
        except Exception:
